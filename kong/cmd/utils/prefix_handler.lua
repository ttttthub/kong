...
local function prepare_prefix(kong_config, nginx_custom_template_path)
  ...
  do
    local i, j = 1, 2
    if kong_config.ssl_cipher_suite == "modern"
    or kong_config.ssl_cipher_suite == "intermediate"
    then
      i, j = j, i
    end
    ...
    for _, target in ipairs({ "proxy", "admin", "status" }) do
      ...
      if ssl_enabled and #ssl_cert == 0 and #ssl_cert_key == 0 then
        ...
        ssl_cert[i] = kong_config[prefix .. "ssl_cert_default"]
        ssl_cert[j] = kong_config[prefix .. "ssl_cert_default_ecdsa"]
        ssl_cert_key[i] = kong_config[prefix .. "ssl_cert_key_default"]
        ssl_cert_key[j] = kong_config[prefix .. "ssl_cert_key_default_ecdsa"]
      end
    end
  end

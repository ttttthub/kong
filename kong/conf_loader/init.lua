...
local function check_and_infer(conf, opts)
  ...
  for _, prefix in ipairs({ "", "admin_", "status_" }) do
    local listen = conf[(prefix == "" and "proxy_" or prefix) .. "listen"]
    local ssl_enabled = (concat(listen, ",") .. " "):find("%sssl[%s,]") ~= nil
    if not ssl_enabled and prefix == "" then
  ...
  if conf.ssl_cipher_suite ~= "custom" then
    ...
    if suite then
      ...
      -- there is no secure predefined one for old at the moment (and it's too slow to generate).
